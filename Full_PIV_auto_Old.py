...
for ind, pic_folder in enumerate(master_pciture_folder, start = first_folder_number-1):
    Expirement_date_pickle = Mf.convert_date(Mf.read_data_from_cihx(pic_folder, '<date>'))
    img_number, real_cor, object_range_cut, prior_knowledge, contrast, bias_x, bias_y, the_selected_image, object_array, PIV_Range = Mf.load_pickle(os.path.join(pickle_save_folder, f'inital_data_{Expirement_date_pickle}.pkl'))[ind]
